...
// Preset network traffic profiles for demo
const PRESETS: Record<string, { label: string; data: Record<string, number> }> = {
  ddos: {
    ...
    data: {
      ...
      total_length_fwd_packets: 5000000,
  ...
  portscan: {
    ...
    data: {
      ...
      total_length_fwd_packets: 44000,
      total_length_bwd_packets: 0,
      fwd_packet_length_max: 44,
      fwd_packet_length_min: 44,
      fwd_packet_length_mean: 44,
  ...
  webattack: {
    ...
    data: {
      ...
      total_backward_packets: 450,
      total_length_fwd_packets: 150000,
      total_length_bwd_packets: 120000,
  ...
  botnet: {
    ...
    data: {
      ...
      total_backward_packets: 110,
      total_length_fwd_packets: 12000,
      total_length_bwd_packets: 11000,
...
export default function PredictPage() {
  ...
  const resultMeta = result ? (RESULT_COLORS[result.prediction] || { color: accent, darkColor: accent, label: result.prediction, icon: "⚠️" }) : null;
      ...
      <main style={{ paddingTop: 80, paddingBottom: 60, paddingLeft: 40, paddingRight: 40, maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ marginTop: 20, marginBottom: 32 }}>
          ...
          <p style={{ fontSize: 13, opacity: 0.55, marginTop: 8, maxWidth: 560 }}>
            Submit network traffic features to the XGBoost classifier. Load a preset profile or manually tune values.
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 24, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ border: `1px solid ${cardBorder}`, borderRadius: 4, padding: 20, background: cardBg }}>
              ...
              <pre
                style={{
                  fontSize: 11, lineHeight: 1.7, opacity: 0.6,
            ...
            {result && resultMeta && (
              <div
                ...
                style={{
                  ...
                  borderRadius: 4,
                  padding: 24,
                  background: dark ? resultMeta.darkColor + "08" : resultMeta.color + "06",
                }}
              >
                <div style={{ fontSize: 32, marginBottom: 8 }}>{resultMeta.icon}</div>
                <div style={{ fontSize: 11, opacity: 0.5, letterSpacing: "0.12em", marginBottom: 6 }}>DETECTED ATTACK</div>
                <div
                  style={{
                ...
                <div style={{ fontSize: 12, opacity: 0.5, lineHeight: 1.7 }}>
                  Raw value: <code style={{ fontFamily: "inherit" }}>{result.prediction}</code>
                  <br />
                  Model: XGBoost Multi-Attack Classifier
                </div>
                <a
                  href={`/threats#${result.prediction.toLowerCase()}`}
                  style={{
                    display: "inline-block", marginTop: 14,
                    fontSize: 12, color: dark ? resultMeta.darkColor : resultMeta.color,
